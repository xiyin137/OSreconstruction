...
1. `BHW.extendedTube_same_sourceGram_extendF_eq`: for `z,w ∈
   ...
   | Low-rank residual-frame alignment | Selected-span frame producer, alignment carrier, and residual-subspace extraction checked in `BHWPermutation/SourceHWLowRankAlignment.lean`. | `HWLowRankSelectedSpanFrameData` stores the scalar rank, selected principal indices, and `HWLowRankSelectedSpanFrame`.  `hw_lowRank_selectedSpanFrame_of_sameSourceGram` chooses a nonzero principal scalar block with `exists_sourcePrincipalMinor_ne_zero_of_sourceSymmetricRank`, uses the checked selected-projection theorem for head orthogonality, and uses the exact-rank Schur-zero theorem on both endpoints to obtain residual-pairing equality as `0 = 0`.  `HWLowRankSelectedSpanAlignment` records the determinant-one selected-span alignment and the left/right residual families in the common orthogonal complement.  `hw_lowRank_residualSubspaces_after_selectedAlignment` extracts totally isotropic residual subspaces orthogonal to the selected block.  The remaining producer starts at building this alignment and then the common isotropic residual frame/dual-frame/null-boost normal form. |
...
| Lemma 2, low rank | Checked data/analytic support: `BHW.ComplexMinkowskiTotallyIsotropicSubspace`, `BHW.complexMinkowskiTotallyIsotropic_span_range`, `BHW.span_frame_orthogonal_to_subspace`, `BHW.HWSameSourceGramSingularContractionData`, `BHW.HWLowRankIsotropicNormalForm`, `BHW.hw_lowRank_isotropicNormalForm_to_contractionData`, `BHW.hw_sameSourceGram_singularLimit_extendF_eq`, `BHW.HWLowRankSelectedSpanFrameData`, `BHW.hw_lowRank_selectedSpanFrame_of_sameSourceGram`, `BHW.HWLowRankSelectedSpanFrame`, `BHW.HWLowRankSelectedSpanAlignment`, and `BHW.hw_lowRank_residualSubspaces_after_selectedAlignment`; remaining geometric producer: `BHW.hw_lowRank_isotropicNormalForm_of_sameSourceGram`, hence `BHW.hw_sameSourceGram_singular_contractionData`. | The residual isotropic-frame geometry, coefficient-freedom extended-tube theorem, null-boost contraction family, and two-curve fields are decomposed into named Lean surfaces.  The low-rank finite-frame span induction, same-Gram selected-span frame producer, selected-span alignment carrier, residual-subspace extraction, normal-form carrier, conversion to contraction data, and two-curve continuity limit inside `ExtendedTube d n` are production Lean in `BHWPermutation/SourceHWSingularLimit.lean` and `BHWPermutation/SourceHWLowRankAlignment.lean`; the remaining work is the finite-dimensional construction of the selected-span Lorentz alignment plus common isotropic residual frame/dual-frame/null-boost normal form. |
