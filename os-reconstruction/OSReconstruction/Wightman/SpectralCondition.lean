...
/-- The forward cone in absolute coordinates: every successive difference,
including the first point relative to zero, lies in the open forward cone. -/
...
/-- Forward-tube analyticity, polynomial real-direction growth uniformly on
compact imaginary-height sets, and full-Schwartz boundary recovery.

This permits singular behavior as the imaginary part approaches the cone
boundary. It does not control growth along unbounded imaginary rays and
does not, by itself, imply positive spectral support. The shared Wightman
records therefore require `SpectralConditionDistribution` separately. -/
