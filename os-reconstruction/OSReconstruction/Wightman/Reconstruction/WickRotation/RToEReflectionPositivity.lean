...
This is the support theorem needed by the R→E quotient pairing. The producer
transports `Wfn.spectral_support` to `section43WightmanSpectralRegion` by the
proved reduced-to-full momentum embedding.
-/
...
/-- The Wightman-side Fourier-Laplace package uses the actual distributional
spectrum condition. Its interior representation follows from boundary
uniqueness and integrable Schwartz slices. -/
noncomputable def rToESection43DualConeFLPackage_of_wightmanFunctions
    (Wfn : WightmanFunctions d) (N : ℕ) :
    RToESection43DualConeFLPackage (d := d) Wfn N := by
  have hC_open := forwardConeFlat_isOpen d N
  have hC_conv := forwardConeFlat_convex d N
  have hC_cone : IsCone ((flattenCLEquivReal N (d + 1)) '' ForwardConeAbs d N) :=
    fun y hy t ht => forwardConeFlat_isCone d N t ht y hy
  have hC_salient := rToEForwardConeFlat_salient (d := d) N
  refine
    { Tflat := rToEFullFrequencyDistribution Wfn N
      hCflat_open := hC_open
      hCflat_conv := hC_conv
      hCflat_cone := hC_cone
      hCflat_salient := hC_salient
      dualSupport := rToEFullFrequencyDistribution_dualSupport Wfn N
      boundary := rToEFullFrequencyDistribution_boundary Wfn N
      fourierLaplace := ?_ }
  intro z hz
  ...
  calc
    F_ext_on_translatedPET_total Wfn z = (W_analytic_BHW Wfn N).val z :=
      F_ext_on_translatedPET_total_eq_on_PET Wfn z hzPET
    _ = (Wfn.spectrum_condition N).choose z :=
      (W_analytic_BHW Wfn N).property.2.1 z hzFT
    _ = _ := rToE_forwardTube_fourierLaplace Wfn N
      hC_open hC_conv hC_cone hC_salient z hzFT

/-- Full Section 4.3 spectral support derived from the Wightman axioms.
This package does not depend on the BHW continuation. -/
noncomputable def rToESection43WightmanSupport_of_wightmanFunctions
    (Wfn : WightmanFunctions d) :
    RToESection43WightmanSupport (d := d) Wfn where
  Tflat := rToEFullFrequencyDistribution Wfn
  support := rToEFullFrequencyDistribution_support Wfn
  boundary := rToEFullFrequencyDistribution_boundary Wfn
