...
import OSReconstruction.Wightman.Reconstruction.WickRotation.RToEForwardTube
...
theorem hasForwardTubeGrowth_of_wightman {d : ℕ} [NeZero d]
  ...
  have h01 : (⟨0, by omega⟩ : Fin n) ≠ (⟨1, hn⟩ : Fin n) := by
    rw [Fin.ne_iff_vne]
    norm_num
  -- Regulated growth comes from the actual supported momentum distribution.
  let W_analytic : (Fin n → Fin (d + 1) → ℂ) → ℂ := (Wfn.spectrum_condition n).choose
  obtain ⟨C_vt, N_vt, q_vt, hC_vt_pos, hVT_bound⟩ :=
    OSReconstruction.rToE_forwardTube_vladimirov_growth Wfn n
  ...
  have hVT_at_y := hVT_bound (fun k => wickRotatePoint (xs' (π' k))) hfwd'
  rw [hnorm_chain]
  have htime_strict : StrictMono (fun k : Fin n => x k 0) := by
    rcases Nat.exists_eq_succ_of_ne_zero (by omega : n ≠ 0) with ⟨m, rfl⟩
    rw [Fin.strictMono_iff_lt_succ]
...
/-- Global a.e. polynomial growth of the Euclidean kernel, weighted by
coincidence distance. The constants depend on the Wightman data and arity,
not on a test function or a later block translation. -/
theorem wick_rotated_kernel_ae_polynomial_growth {d n : ℕ} [NeZero d]
    (Wfn : WightmanFunctions d) :
    ∃ (C : ℝ) (N q : ℕ), 0 < C ∧
      ∀ᵐ (x : NPointDomain d n) ∂MeasureTheory.volume,
        ‖F_ext_on_translatedPET_total Wfn (fun k => wickRotatePoint (x k))‖ *
  ...
  refine ⟨C_bd * nR ^ N, N, q, by dsimp [nR]; positivity, ?_⟩
...
theorem wick_rotated_kernel_mul_zeroDiagonal_integrable {d n : ℕ} [NeZero d]
  ...
  · obtain ⟨C_bd, N, q, hC_pos, hgrowth⟩ := wick_rotated_kernel_ae_polynomial_growth Wfn
    exact kernel_mul_zeroDiagonal_integrable_of_ae_infDist_mul_pow_le_polynomial
      (hK_meas := bhw_euclidean_kernel_measurable Wfn) (f := f)
      (m := q) (M := N) (hcoin := hcoin) (C_bd := C_bd)
      (hC := hC_pos.le) (hK_bound := hgrowth)
