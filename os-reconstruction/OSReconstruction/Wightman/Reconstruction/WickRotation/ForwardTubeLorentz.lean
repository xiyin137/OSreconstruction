...
/-- Compact-height growth is preserved by real Lorentz precomposition. -/
theorem forward_tube_lorentz_compact_growth {d n : ℕ} [NeZero d]
    (Λ : LorentzGroup d)
    (F : (Fin n → Fin (d + 1) → ℂ) → ℂ)
    (hF : ∀ (K : Set (Fin n → Fin (d + 1) → ℝ)), IsCompact K →
      ...
      ∃ (C : ℝ) (N : ℕ), 0 < C ∧
        ∀ (x y : Fin n → Fin (d + 1) → ℝ), y ∈ K →
          ‖F (fun k μ => ∑ ν, (Λ.val μ ν : ℂ) *
            ((x k ν : ℂ) + (y k ν : ℂ) * Complex.I))‖ ≤
              C * (1 + ‖x‖) ^ N := by
  let L : NPointDomain d n →L[ℝ] NPointDomain d n :=
    LinearMap.toContinuousLinearMap
      { toFun := fun x k μ => ∑ ν, Λ.val μ ν * x k ν
        map_add' := by
          intro x y
          ext k μ
  ...
  intro K hK hsub
  have hLK : L '' K ⊆ ForwardConeAbs d n := by
    rintro _ ⟨y, hy, rfl⟩
    have hyc : (fun k μ => (y k μ : ℂ) * Complex.I) ∈ ForwardTube d n := by
      rw [forwardTube_eq_imPreimage]
      simpa using hsub hy
    have hLc := restricted_preserves_forward_tube Λ _ hyc
    rw [forwardTube_eq_imPreimage] at hLc
    simpa [L, Complex.im_sum, Complex.mul_im] using hLc
  obtain ⟨C, N, hC, hbound⟩ := hF (L '' K) (hK.image L.continuous) hLK
  refine ⟨C * (1 + ‖L‖) ^ N, N, by positivity, ?_⟩
  intro x y hy
  have harg :
      (fun k μ => ∑ ν, (Λ.val μ ν : ℂ) *
        ((x k ν : ℂ) + (y k ν : ℂ) * Complex.I)) =
      (fun k μ => (L x k μ : ℂ) + (L y k μ : ℂ) * Complex.I) := by
    ext k μ
    change (∑ ν, (Λ.val μ ν : ℂ) *
        ((x k ν : ℂ) + (y k ν : ℂ) * Complex.I)) =
      ((∑ ν, Λ.val μ ν * x k ν : ℝ) : ℂ) +
        ((∑ ν, Λ.val μ ν * y k ν : ℝ) : ℂ) * Complex.I
    simp [mul_add, Finset.sum_add_distrib, Finset.sum_mul, mul_assoc]
  have hnorm : 1 + ‖L x‖ ≤ (1 + ‖L‖) * (1 + ‖x‖) := by
    nlinarith [L.le_opNorm x, norm_nonneg L, norm_nonneg x]
  rw [harg]
  calc
    ‖F (fun k μ => (L x k μ : ℂ) + (L y k μ : ℂ) * Complex.I)‖
        ≤ C * (1 + ‖L x‖) ^ N := hbound (L x) (L y) ⟨y, hy, rfl⟩
    _ ≤ C * ((1 + ‖L‖) * (1 + ‖x‖)) ^ N := by gcongr
    _ = C * (1 + ‖L‖) ^ N * (1 + ‖x‖) ^ N := by rw [mul_pow, mul_assoc]
...
theorem forward_tube_bv_integrable_of_compact {d n : ℕ} [NeZero d]
    (F : (Fin n → Fin (d + 1) → ℂ) → ℂ)
    (hF : DifferentiableOn ℂ F (ForwardTube d n))
    ...
    MeasureTheory.Integrable
      ...
      MeasureTheory.volume := by
  have hy : ε • η ∈ ForwardConeAbs d n :=
    forwardConeAbs_smul d n ε hε η hη
  obtain ⟨C, N, hC, hbound⟩ :=
    hF_growth {ε • η} isCompact_singleton (Set.singleton_subset_iff.mpr hy)
  refine polynomial_growth_mul_schwartz_integrable _
    (forward_tube_slice_aestrongly_measurable F hF η hη ε hε) C N hC ?_ f
  intro x
  simpa [Pi.smul_apply, smul_eq_mul, Complex.ofReal_mul, mul_assoc] using
    hbound x (ε • η) (Set.mem_singleton _)
