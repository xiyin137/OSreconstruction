...
/-- A function bounded by `M` on the support of a real nonnegative normalized
Schwartz kernel has convolution average bounded by `M`. -/
theorem norm_integral_mul_le_of_tsupport_bound
    (ψ : SchwartzMap (Fin m → ℝ) ℂ)
    (F : (Fin m → ℝ) → ℂ)
    (M : ℝ)
    (hψ_nonneg : ∀ t : Fin m → ℝ, 0 ≤ (ψ t).re)
    (hψ_real : ∀ t : Fin m → ℝ, (ψ t).im = 0)
    (hψ_norm : ∫ t : Fin m → ℝ, ψ t = 1)
    (hF : ∀ t ∈ tsupport (ψ : (Fin m → ℝ) → ℂ), ‖F t‖ ≤ M) :
    ‖∫ t : Fin m → ℝ, F t * ψ t‖ ≤ M := by
  have hnorm_int : ∫ t : Fin m → ℝ, ‖ψ t‖ = 1 :=
    integral_norm_eq_one_of_real_nonneg_normalized
      ψ hψ_nonneg hψ_real hψ_norm
  have hpoint :
      ∀ t : Fin m → ℝ, ‖F t * ψ t‖ ≤ M * ‖ψ t‖ := by
    intro t
    by_cases ht : ψ t = 0
    · simp [ht]
    · have ht_support :
          t ∈ Function.support (ψ : (Fin m → ℝ) → ℂ) := by
        simpa [Function.mem_support] using ht
      rw [norm_mul]
      exact mul_le_mul_of_nonneg_right
        (hF t (subset_tsupport _ ht_support)) (norm_nonneg _)
  calc
    ‖∫ t : Fin m → ℝ, F t * ψ t‖
        ≤ ∫ t : Fin m → ℝ, M * ‖ψ t‖ := by
          exact norm_integral_le_of_norm_le
            (ψ.integrable.norm.const_mul M)
            (Filter.Eventually.of_forall hpoint)
    _ = M * ∫ t : Fin m → ℝ, ‖ψ t‖ := by
      rw [MeasureTheory.integral_const_mul]
    _ = M := by simp [hnorm_int]
