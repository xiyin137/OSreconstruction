...
/-- A uniformly equicontinuous family of distributionally holomorphic chart
distributions has canonical holomorphic representatives uniformly bounded on
every compact subset of the common domain. -/
theorem distributionalHolomorphic_regular_uniform_compact_bound
    (T : ℕ → SchwartzMap (ComplexChartSpace m) ℂ →L[ℂ] ℂ)
    {U K : Set (ComplexChartSpace m)}
    (hm : 0 < m)
    (hU_open : IsOpen U)
    ...
    (C : ℝ)
    (hC : 0 ≤ C)
    (hT : ∀ level (φ : SchwartzMap (ComplexChartSpace m) ℂ),
      ‖T level φ‖ ≤
        C * s.sup
          (schwartzSeminormFamily ℂ (ComplexChartSpace m) ℂ) φ)
    (hCR : ∀ level, IsDistributionalHolomorphicOn (T level) U) :
    ∃ H : ℕ → ComplexChartSpace m → ℂ,
      (∀ level, DifferentiableOn ℂ (H level) U) ∧
      (∀ level,
        RepresentsDistributionOnComplexDomain (T level) (H level) U) ∧
      ∃ M : ℝ, 0 ≤ M ∧
        ∀ level z, z ∈ K → ‖H level z‖ ≤ M := by
  classical
  let E := EuclideanSpace ℝ (Fin (m * 2))
  let e := complexChartEuclideanCLE m
  let V : Set E := e '' U
  let KE : Set E := e '' K
  let TE : ℕ → SchwartzMap E ℂ →L[ℂ] ℂ :=
    fun level => transportedDistributionToEuclidean (T level)
  haveI : Nonempty (Fin (m * 2)) := by
    exact ⟨⟨0, Nat.mul_pos hm (by norm_num)⟩⟩
  have hV_open : IsOpen V := by
    simpa [V, e] using e.toHomeomorph.isOpenMap U hU_open
  have hKE_compact : IsCompact KE := by
    exact hK_compact.image e.continuous
  have hKE_subset : KE ⊆ V := by
    rintro x ⟨z, hz, rfl⟩
    exact ⟨z, hK_subset hz, rfl⟩
  let B :
      SchwartzMap E ℂ →L[ℂ] SchwartzMap (ComplexChartSpace m) ℂ :=
    ((complexChartEuclideanSchwartzCLE m).symm :
      SchwartzMap E ℂ →L[ℂ] SchwartzMap (ComplexChartSpace m) ℂ)
  obtain ⟨sE, CB, hCB, hB⟩ :=
    SchwartzMap.exists_schwartzCLM_finsetSeminormBound_between B s
  have hTE_bound :
      ∀ level (φ : SchwartzMap E ℂ),
        ‖TE level φ‖ ≤
          (C * CB) *
            sE.sup (schwartzSeminormFamily ℂ E ℂ) φ := by
    intro level φ
    have hT' := hT level (B φ)
    have hB' := hB φ
    calc
      ‖TE level φ‖ = ‖T level (B φ)‖ := by
        rfl
      _ ≤ C * s.sup
          (schwartzSeminormFamily ℂ (ComplexChartSpace m) ℂ)
            (B φ) := hT'
      _ ≤ C * (CB *
          sE.sup (schwartzSeminormFamily ℂ E ℂ) φ) :=
        mul_le_mul_of_nonneg_left hB' hC
      _ = (C * CB) *
          sE.sup (schwartzSeminormFamily ℂ E ℂ) φ := by ring
  have hΔE :
      ∀ level (ψ : SchwartzMap E ℂ),
        SupportsInOpen (ψ : E → ℂ) V →
          TE level
            (LineDeriv.laplacianCLM ℝ E (SchwartzMap E ℂ) ψ) = 0 := by
    intro level ψ hψ
    let ψc : SchwartzMap (ComplexChartSpace m) ℂ :=
      (complexChartEuclideanSchwartzCLE m).symm ψ
    have hψc : SupportsInOpen (ψc : ComplexChartSpace m → ℂ) U := by
      simpa [ψc, V, E, e] using
        (supportsInOpen_transport_to_euclidean
          (m := m) (φ := ψ) (U0 := U) hψ)
    have hzero :
        T level (complexChartLaplacianSchwartzCLM ψc) = 0 :=
      local_laplacian_zero_of_distributionalHolomorphic
        (T level) (hCR level) ψc hψc
    have htransport :
        (complexChartEuclideanSchwartzCLE m).symm
            (LineDeriv.laplacianCLM ℝ E (SchwartzMap E ℂ) ψ) =
          complexChartLaplacianSchwartzCLM ψc := by
      ...
      simpa [ψc, E] using
        (complexChartLaplacianSchwartzCLM_transport (m := m) ψc).symm
    change
      T level ((complexChartEuclideanSchwartzCLE m).symm
        (LineDeriv.laplacianCLM ℝ E (SchwartzMap E ℂ) ψ)) = 0
    rw [htransport]
    exact hzero
  let HE : ℕ → E → ℂ := fun level =>
    euclideanWeylOpenRepresentative (TE level) hV_open
  let H : ℕ → ComplexChartSpace m → ℂ := fun level z =>
    HE level (e z)
  have hH_smooth :
      ∀ level, ContDiffOn ℝ (⊤ : ℕ∞) (H level) U := by
    intro level
    have hHE_smooth :
        ContDiffOn ℝ (⊤ : ℕ∞) (HE level) V := by
      simpa [HE] using
        contDiffOn_euclideanWeylOpenRepresentative
          (TE level) hV_open (hΔE level)
    exact
      hHE_smooth.comp
        e.contDiff.contDiffOn
  ...
  have hH_rep :
      ∀ level,
        RepresentsDistributionOnComplexDomain
          (T level) (H level) U := by
    intro level
    have hHE_rep :
        RepresentsEuclideanDistributionOn
          (TE level) (HE level) V := by
      simpa [HE] using
        euclideanWeylOpenRepresentative_represents
          (TE level) hV_open (hΔE level)
    simpa [H, HE, TE, V, E, e] using
      (representsDistributionOnComplexDomain_of_euclidean
        (m := m) (T level) (U0 := U) (HE level) hHE_rep)
  have hH_holo :
      ∀ level, DifferentiableOn ℂ (H level) U := by
    intro level
    have hDbar :
        ∀ j : Fin m, ∀ z ∈ U, pointwiseDbar j (H level) z = 0 :=
      pointwiseDbar_eq_zero_of_distributionalHolomorphic
        (T level) hU_open (hCR level)
        (H level) (hH_smooth level) (hH_rep level)
    exact differentiableOn_complex_of_contDiffOn_real_and_pointwiseDbar_zero
      hU_open (hH_smooth level) hDbar
  obtain ⟨M, hM, hbound⟩ :=
    euclideanWeylOpenRepresentative_uniform_compact_bound
      TE hV_open hKE_compact hKE_subset sE (C * CB)
      (mul_nonneg hC hCB) hTE_bound hΔE
  refine ⟨H, hH_holo, hH_rep, M, hM, ?_⟩
  intro level z hz
  exact hbound level (e z) ⟨z, hz, rfl⟩
