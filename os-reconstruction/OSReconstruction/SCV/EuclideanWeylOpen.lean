...
/-- A family of Euclidean distributions with one common finite Schwartz
seminorm bound has uniformly bounded canonical Weyl representatives on every
compact subset of the common harmonicity domain. -/
theorem euclideanWeylOpenRepresentative_uniform_compact_bound
    {ι : Type*} [Fintype ι] [Nonempty ι]
    (T : ℕ → SchwartzMap (EuclideanSpace ℝ ι) ℂ →L[ℂ] ℂ)
    {V K : Set (EuclideanSpace ℝ ι)}
    (hV_open : IsOpen V)
    ...
    (C : ℝ)
    (hC : 0 ≤ C)
    (hT : ∀ level (φ : SchwartzMap (EuclideanSpace ℝ ι) ℂ),
      ‖T level φ‖ ≤
        C * s.sup
          (schwartzSeminormFamily ℂ (EuclideanSpace ℝ ι) ℂ) φ)
    (hΔ :
      ∀ level (φ : SchwartzMap (EuclideanSpace ℝ ι) ℂ),
        SupportsInOpen
          (φ : EuclideanSpace ℝ ι → ℂ) V →
          T level
    ...
    ∃ M : ℝ, 0 ≤ M ∧
      ∀ level x, x ∈ K →
        ‖euclideanWeylOpenRepresentative
            (T level) hV_open x‖ ≤ M := by
  classical
  let p :=
    schwartzSeminormFamily ℂ (EuclideanSpace ℝ ι) ℂ
  let β := {x : EuclideanSpace ℝ ι // x ∈ K}
  let Uβ : β → Set (EuclideanSpace ℝ ι) := fun a =>
    Metric.ball (a : EuclideanSpace ℝ ι)
      (euclideanWeylOpenRadius hV_open
        ⟨(a : EuclideanSpace ℝ ι), hK_subset a.property⟩ / 4)
  ...
  have hK_cover : K ⊆ ⋃ a : β, Uβ a := by
    ...
    have hRpos :=
      euclideanWeylOpenRadius_pos hV_open ⟨x, hK_subset hx⟩
    simp [Uβ, hRpos]
  obtain ⟨t, htcover⟩ :=
    hK_compact.elim_finite_subcover Uβ hUβ_open hK_cover
  have hlocal :
      ∀ a : β, ∃ M : ℝ, 0 ≤ M ∧
        ∀ x ∈ K,
          s.sup p
              (euclideanReflectedTranslate x
                (euclideanWeylBump
                  ...
                  (by
                    have hRpos :=
                      euclideanWeylOpenRadius_pos hV_open
                        ⟨(a : EuclideanSpace ℝ ι), hK_subset a.property⟩
                    positivity))) ≤ M := by
    intro a
    let R :=
      euclideanWeylOpenRadius hV_open
    ...
    have hε : 0 < ε := by
      dsimp [ε]
      positivity
    let ρ : SchwartzMap (EuclideanSpace ℝ ι) ℂ :=
      euclideanWeylBump (ι := ι) ε hε
    have hρ_compact :
        HasCompactSupport (ρ : EuclideanSpace ℝ ι → ℂ) := by
      exact euclideanWeylBump_hasCompactSupport ε hε
    let q : EuclideanSpace ℝ ι → ℝ := fun x =>
      ∑ i ∈ s, p i (euclideanReflectedTranslate x ρ)
    have hq_cont : Continuous q := by
      apply continuous_finset_sum
      intro i _hi
      exact
        ((schwartz_withSeminorms ℂ
            (EuclideanSpace ℝ ι) ℂ).continuous_seminorm i).comp
          (continuous_euclideanReflectedTranslate_of_isCompactSupport
            ρ hρ_compact)
    obtain ⟨M0, hM0⟩ :=
      hK_compact.exists_bound_of_continuousOn hq_cont.continuousOn
    let M := max M0 0
    refine ⟨M, by simp [M], ?_⟩
    intro x hx
    have hsup_le_sum :
        s.sup p (euclideanReflectedTranslate x ρ) ≤ q x := by
      have hle := Seminorm.le_def.mp
        (Seminorm.finset_sup_le_sum p s)
        (euclideanReflectedTranslate x ρ)
      calc
        s.sup p (euclideanReflectedTranslate x ρ)
            ≤ (∑ i ∈ s, p i)
    ...
    have hqx_nonneg : 0 ≤ q x := by
      dsimp [q]
      positivity
    have hqx_le : q x ≤ M0 := by
      simpa [Real.norm_eq_abs, abs_of_nonneg hqx_nonneg] using hM0 x hx
    have hrewrite :
        euclideanWeylBump
            (euclideanWeylOpenRadius hV_open
              ⟨(a : EuclideanSpace ℝ ι), hK_subset a.property⟩ / 4)
      ...
      rfl
    rw [hrewrite]
    exact hsup_le_sum.trans (hqx_le.trans (le_max_left M0 0))
  choose Mloc hMloc_nonneg hMloc using hlocal
  let Msum : ℝ := t.sum Mloc
  have hMsum_nonneg : 0 ≤ Msum := by
    exact Finset.sum_nonneg fun a _ha => hMloc_nonneg a
  refine ⟨C * Msum, mul_nonneg hC hMsum_nonneg, ?_⟩
  intro level x hx
  have hxcover := htcover hx
  simp only [mem_iUnion] at hxcover
  obtain ⟨a, ha, hxa⟩ := hxcover
  let R :=
    euclideanWeylOpenRadius hV_open
      ⟨(a : EuclideanSpace ℝ ι), hK_subset a.property⟩
  have hRpos : 0 < R :=
  ...
  have hΔball :
      ∀ φ : SchwartzMap (EuclideanSpace ℝ ι) ℂ,
        SupportsInOpen
          ...
          (Metric.ball (a : EuclideanSpace ℝ ι) R) →
          T level
            (LineDeriv.laplacianCLM ℝ
              (EuclideanSpace ℝ ι)
              (SchwartzMap (EuclideanSpace ℝ ι) ℂ) φ) = 0 := by
  ...
  have hopen :
      euclideanWeylOpenRepresentative
        ...
        euclideanWeylBallRepresentative
          (T level) (a : EuclideanSpace ℝ ι) R x := by
    exact
      euclideanWeylOpenRepresentative_eq_ballRepresentative_on_small_ball
        (T level) hV_open (hΔ level)
        (hK_subset a.property) x (by simpa [Uβ, R] using hxa)
  have hball :
      euclideanWeylBallRepresentative
          (T level) (a : EuclideanSpace ℝ ι) R x =
        T level
          (euclideanReflectedTranslate x
            (euclideanWeylBump ε hε)) := by
    apply euclideanWeylBallRepresentative_eq_regularized_on_ball
      (T level) (r := R / 4) (R := R) (ε := ε) hε
    · dsimp [ε]
      linarith
    · exact hΔball
    · simpa [Uβ, R] using hxa
  have htest_bound :
      s.sup p
          (euclideanReflectedTranslate x
            (euclideanWeylBump ε hε)) ≤ Mloc a := by
    simpa [R, ε, p] using hMloc a x hx
  have hMloc_sum : Mloc a ≤ Msum := by
    exact Finset.single_le_sum
      (fun b hb => hMloc_nonneg b) ha
  calc
    ‖euclideanWeylOpenRepresentative
        (T level) hV_open x‖
        =
      ‖T level
        (euclideanReflectedTranslate x
          (euclideanWeylBump ε hε))‖ := by rw [hopen, hball]
    _ ≤ C * s.sup p
        (euclideanReflectedTranslate x
          (euclideanWeylBump ε hε)) := by
            simpa [p] using
    ...
    _ ≤ C * Mloc a := mul_le_mul_of_nonneg_left htest_bound hC
    _ ≤ C * Msum := mul_le_mul_of_nonneg_left hMloc_sum hC
...
/-- The canonical open-set Weyl representative itself represents the original
distribution on every compactly supported test inside the harmonicity
domain. -/
theorem euclideanWeylOpenRepresentative_represents
    {ι : Type*} [Fintype ι] [Nonempty ι]
    (T : SchwartzMap (EuclideanSpace ℝ ι) ℂ →L[ℂ] ℂ)
    ...
    RepresentsEuclideanDistributionOn T
      (euclideanWeylOpenRepresentative T hV_open) V := by
  let H : EuclideanSpace ℝ ι → ℂ :=
    euclideanWeylOpenRepresentative T hV_open
  have hHsmooth : ContDiffOn ℝ (⊤ : ℕ∞) H V := by
    simpa [H] using
      contDiffOn_euclideanWeylOpenRepresentative T hV_open hΔ
  intro φ hφ
  apply euclideanDistribution_representation_of_local_representations_for_test
    T H φ hφ.1
  intro x hx
  let R : ℝ := euclideanWeylOpenRadius hV_open ⟨x, hφ.2 hx⟩
  have hRpos : 0 < R :=
    euclideanWeylOpenRadius_pos hV_open ⟨x, hφ.2 hx⟩
  let U : Set (EuclideanSpace ℝ ι) := Metric.ball x (R / 4)
  have hU_open : IsOpen U := isOpen_ball
  have hxU : x ∈ U := by
    simp [U, hRpos]
  have hU_subset_V : U ⊆ V := by
    ...
    have hyR : y ∈ Metric.closedBall x R := by
      ...
      nlinarith
    exact euclideanWeylOpenRadius_closedBall_subset
      hV_open ⟨x, hφ.2 hx⟩ hyR
  have hH_cont : ContinuousOn H U :=
    hHsmooth.continuousOn.mono hU_subset_V
  refine ⟨U, hU_open, hxU, hH_cont, ?_⟩
  intro ψ hψ
  have hΔx :
      ∀ θ : SchwartzMap (EuclideanSpace ℝ ι) ℂ,
        SupportsInOpen
          (θ : EuclideanSpace ℝ ι → ℂ) (Metric.ball x R) →
          T
            (LineDeriv.laplacianCLM ℝ (EuclideanSpace ℝ ι)
              (SchwartzMap (EuclideanSpace ℝ ι) ℂ) θ) = 0 := by
    intro θ hθ
    apply hΔ θ
    refine ⟨hθ.1, hθ.2.trans ?_⟩
    exact Metric.ball_subset_closedBall.trans
      (euclideanWeylOpenRadius_closedBall_subset
        hV_open ⟨x, hφ.2 hx⟩)
  have hball_rep :
      RepresentsEuclideanDistributionOn T
        (euclideanWeylBallRepresentative T x R)
        (Metric.ball x (R / 4)) := by
    exact euclideanWeylBallRepresentative_represents_on_ball
      T x (by linarith) (by linarith) hΔx
  have hψ_ball :
      SupportsInOpen
        (ψ : EuclideanSpace ℝ ι → ℂ)
        (Metric.ball x (R / 4)) := by
    simpa [U] using hψ
  ...
  calc
    T ψ =
        ∫ y : EuclideanSpace ℝ ι,
          euclideanWeylBallRepresentative T x R y * ψ y := hEq
    _ = ∫ y : EuclideanSpace ℝ ι, H y * ψ y := by
      apply MeasureTheory.integral_congr_ae
      filter_upwards with y
      by_cases hy : y ∈ tsupport (ψ : EuclideanSpace ℝ ι → ℂ)
      · have hyU : y ∈ U := hψ.2 hy
        have hlocal :=
          euclideanWeylOpenRepresentative_eq_ballRepresentative_on_small_ball
            T hV_open hΔ (hφ.2 hx) y
              (by simpa [U, R] using hyU)
        have hHy :
            H y = euclideanWeylBallRepresentative T x R y := by
          simpa [H, R] using hlocal
        rw [hHy]
      · have hψy : ψ y = 0 := image_eq_zero_of_notMem_tsupport hy
        simp [hψy]
