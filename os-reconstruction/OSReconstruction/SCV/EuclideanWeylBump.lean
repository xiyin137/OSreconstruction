...
/-- The Euclidean Weyl bump is compactly supported. -/
theorem euclideanWeylBump_hasCompactSupport
    {ι : Type*} [Fintype ι]
    (ε : ℝ) (hε : 0 < ε) :
    HasCompactSupport
      (euclideanWeylBump ε hε : EuclideanSpace ℝ ι → ℂ) :=
  IsCompact.of_isClosed_subset
    (isCompact_closedBall (0 : EuclideanSpace ℝ ι) ε)
    (isClosed_tsupport _)
    (euclideanWeylBump_support ε hε)
