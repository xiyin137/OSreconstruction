...
theorem holomorphic_eq_of_eq_on_real_of_connected_finite
    ...
    {x₀ : κ → ℝ}
    (hx₀ : (fun i => (x₀ i : ℂ)) ∈ U)
    (hFG_real : ∀ x : κ → ℝ, (fun i => (x i : ℂ)) ∈ U →
      F (fun i => (x i : ℂ)) = G (fun i => (x i : ℂ))) :
    ∀ z ∈ U, F z = G z := by
  ...
  let F' : (Fin (Fintype.card κ) → ℂ) → ℂ := F ∘ L.symm
  let G' : (Fin (Fintype.card κ) → ℂ) → ℂ := G ∘ L.symm
  have hV_open : IsOpen V := hU_open.preimage L.symm.continuous
  have hV_conn : IsConnected V :=
    L.symm.toHomeomorph.isConnected_preimage.mpr hU_conn
  have hF' : DifferentiableOn ℂ F' V :=
    hF.comp L.symm.differentiable.differentiableOn (mapsTo_preimage _ _)
  have hG' : DifferentiableOn ℂ G' V :=
    hG.comp L.symm.differentiable.differentiableOn (mapsTo_preimage _ _)
  let x₀' : Fin (Fintype.card κ) → ℝ := fun j => x₀ (e.symm j)
  have hL_x₀ :
      L.symm (realToComplex x₀') = fun i => (x₀ i : ℂ) := by
    ext i
    change (realToComplex x₀') (e i) = (x₀ i : ℂ)
    simp [x₀', realToComplex]
  have hx₀' :
      realToComplex x₀' ∈ V := by
    simpa [V, hL_x₀] using hx₀
  have hreal' :
      ∀ x : Fin (Fintype.card κ) → ℝ, realToComplex x ∈ V →
        F' (realToComplex x) = G' (realToComplex x) := by
    intro x hx
    let xκ : κ → ℝ := fun i => x (e i)
    have hL_x :
        L.symm (realToComplex x) = fun i => (xκ i : ℂ) := by
      ext i
      change (realToComplex x) (e i) = (xκ i : ℂ)
      simp [xκ, realToComplex]
    have hxκ :
        (fun i => (xκ i : ℂ)) ∈ U := by
      simpa [V, hL_x] using hx
    simpa [F', G', hL_x] using hFG_real xκ hxκ
  intro z hz
  have hz' : L z ∈ V := by
    simpa [V] using hz
  have heq :=
    holomorphic_eq_of_eq_on_real_of_connected
      hV_open hV_conn hF' hG' hx₀' hreal' (L z) hz'
  simpa [F', G'] using heq

/-- Two holomorphic scalar functions on a connected finite-dimensional
complex domain agree everywhere if they agree on any nonempty open patch of
the totally real locus contained in that domain. -/
theorem holomorphic_eq_of_eq_on_open_real_of_connected_finite
    {κ : Type*} [Fintype κ] [DecidableEq κ]
    {U : Set (κ → ℂ)}
    (hU_open : IsOpen U) (hU_conn : IsConnected U)
    {F G : (κ → ℂ) → ℂ}
  ...
  let L : (κ → ℂ) ≃L[ℂ] (Fin (Fintype.card κ) → ℂ) :=
    ContinuousLinearEquiv.piCongrLeft ℂ
      (fun _ : Fin (Fintype.card κ) => ℂ) e
  let reindex : (Fin (Fintype.card κ) → ℝ) → (κ → ℝ) :=
    fun x i => x (e i)
  let U' : Set (Fin (Fintype.card κ) → ℂ) := L.symm ⁻¹' U
  let V' : Set (Fin (Fintype.card κ) → ℝ) := reindex ⁻¹' V
  let F' : (Fin (Fintype.card κ) → ℂ) → ℂ := F ∘ L.symm
  let G' : (Fin (Fintype.card κ) → ℂ) → ℂ := G ∘ L.symm
  have hU'_open : IsOpen U' := hU_open.preimage L.symm.continuous
  have hU'_conn : IsConnected U' :=
    L.symm.toHomeomorph.isConnected_preimage.mpr hU_conn
  ...
  have hV'_open : IsOpen V' :=
    hV_open.preimage hreindex_cont
  have hV'_ne : V'.Nonempty := by
    obtain ⟨x, hx⟩ := hV_ne
    let x' : Fin (Fintype.card κ) → ℝ :=
      fun j => x (e.symm j)
    refine ⟨x', ?_⟩
    have hreindex : reindex x' = x := by
      funext i
      simp [reindex, x']
    simpa [V', hreindex] using hx
  have hF' : DifferentiableOn ℂ F' U' :=
    hF.comp L.symm.differentiable.differentiableOn
      (mapsTo_preimage _ _)
  have hG' : DifferentiableOn ℂ G' U' :=
    hG.comp L.symm.differentiable.differentiableOn
      (mapsTo_preimage _ _)
  have hL_real :
      ∀ x : Fin (Fintype.card κ) → ℝ,
        L.symm (realToComplex x) =
          fun i => ((reindex x) i : ℂ) := by
    intro x
    ext i
    change (realToComplex x) (e i) = (x (e i) : ℂ)
    rfl
  have hV'_sub :
      ∀ x ∈ V', realToComplex x ∈ U' := by
    intro x hx
    have hxV : reindex x ∈ V := hx
    simpa [U', hL_real x] using hV_sub (reindex x) hxV
  have hzero :
      ∀ x ∈ V', (F' - G') (realToComplex x) = 0 := by
    intro x hx
    have hxV : reindex x ∈ V := hx
    simp [F', G', hL_real x, hFG_real (reindex x) hxV]
  have hident :=
    identity_theorem_totally_real
      hU'_open hU'_conn (hF'.sub hG')
      hV'_open hV'_ne hV'_sub hzero
  intro z hz
  have hz' : L z ∈ U' := by
    simpa [U'] using hz
  have heq := hident (L z) hz'
  simpa [F', G'] using sub_eq_zero.mp heq

/-- Totally-real uniqueness for a nested finite product of complex
coordinates. This avoids repeatedly flattening source/block parameter spaces
by hand before applying the finite-index identity theorem. -/
theorem holomorphic_eq_of_eq_on_real_of_connected_finite_product
  ...
  let L :
    ...
    LinearEquiv.toContinuousLinearEquiv
      (LinearEquiv.piCurry ℂ (fun _ : κ => fun _ : ι => ℂ))
  let V : Set ((Σ _ : κ, ι) → ℂ) := L ⁻¹' U
  let F' : ((Σ _ : κ, ι) → ℂ) → ℂ := F ∘ L
  let G' : ((Σ _ : κ, ι) → ℂ) → ℂ := G ∘ L
  have hV_open : IsOpen V := hU_open.preimage L.continuous
  have hV_conn : IsConnected V :=
    L.toHomeomorph.isConnected_preimage.mpr hU_conn
  have hF' : DifferentiableOn ℂ F' V :=
    hF.comp L.differentiable.differentiableOn
      (Set.mapsTo_preimage L U)
  have hG' : DifferentiableOn ℂ G' V :=
    hG.comp L.differentiable.differentiableOn
      (Set.mapsTo_preimage L U)
  let x₀' : (Σ _ : κ, ι) → ℝ := fun p => x₀ p.1 p.2
  have hL_x₀ :
      L (fun p => (x₀' p : ℂ)) =
        fun i j => (x₀ i j : ℂ) := by
    rfl
      ...
      ∀ x : (Σ _ : κ, ι) → ℝ,
        (fun p => (x p : ℂ)) ∈ V →
          F' (fun p => (x p : ℂ)) =
            G' (fun p => (x p : ℂ)) := by
    intro x hx
    let xκι : κ → ι → ℝ := fun i j => x ⟨i, j⟩
    have hL_x :
        L (fun p => (x p : ℂ)) =
          fun i j => (xκι i j : ℂ) := by
      rfl
    have hxκι :
        (fun i j => (xκι i j : ℂ)) ∈ U := by
      simpa [V, hL_x] using hx
    simpa [F', G', hL_x] using hFG_real xκι hxκι
  intro z hz
  have hz' : L.symm z ∈ V := by
    simpa [V] using hz
  have heq :=
    holomorphic_eq_of_eq_on_real_of_connected_finite
      hV_open hV_conn hF' hG' hx₀' hreal'
      (L.symm z) hz'
  simpa [F', G'] using heq
