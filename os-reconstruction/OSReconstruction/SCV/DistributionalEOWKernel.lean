...
/-- A continuous linear map between Schwartz spaces transports any finite
family of output Schwartz seminorms to a finite family of input seminorms. -/
theorem SchwartzMap.exists_schwartzCLM_finsetSeminormBound_between
    {E F : Type*}
    [NormedAddCommGroup E] [NormedSpace ℝ E]
    [NormedAddCommGroup F] [NormedSpace ℝ F]
    (T : SchwartzMap E ℂ →L[ℂ] SchwartzMap F ℂ)
    (s0 : Finset (ℕ × ℕ)) :
    ∃ s : Finset (ℕ × ℕ), ∃ C : ℝ, 0 ≤ C ∧
      ∀ ψ : SchwartzMap E ℂ,
        s0.sup (schwartzSeminormFamily ℂ F ℂ) (T ψ) ≤
          C * s.sup (schwartzSeminormFamily ℂ E ℂ) ψ := by
  let pE := schwartzSeminormFamily ℂ E ℂ
  let pF := schwartzSeminormFamily ℂ F ℂ
  have hbounded : Seminorm.IsBounded pE pF T.toLinearMap := by
    intro i
    let qi : Seminorm ℂ (SchwartzMap E ℂ) := (pF i).comp T.toLinearMap
    have hqi_cont : Continuous qi := by
      exact ((schwartz_withSeminorms ℂ F ℂ).continuous_seminorm i).comp T.continuous
    obtain ⟨s, C, _hCne, hbound⟩ :=
      Seminorm.bound_of_continuous (schwartz_withSeminorms ℂ E ℂ) qi hqi_cont
    exact ⟨s, C, hbound⟩
