...
to the current production route as of 2026-03-31. The active $E \to R$ story is
now:
\begin{center}
...
$\to$
\leanfile{SCV/TubeBoundaryValues.lean}
$\to$
\leanfile{OSToWightmanBoundaryValues.lean}
\end{center}
The earlier issue-\#48 blocker through the false
\leanname{HasFourierLaplaceReprTempered.uniform\_bound} abstraction is no longer
on the active OS path. The $k=2$ VI.1 files remain important support and local
frontier infrastructure, but they are no longer the first top-level blocker for
\leanname{os\_to\_wightman\_full}.
...
\subsection{Current Status Note (2026-03-31)}

The repository has now undergone two substantial corrections on the
$E \to R$ side.

\begin{itemize}
\item The production route now follows the OS papers strictly: semigroup/Hilbert
  space first, then the several-complex-variables continuation witness, then
  the pure tube-domain boundary-value theorem, and only afterwards the
  Wightman-axiom transfer chain.
\item The older specialized two-point kernel / \texttt{differenceLift} detours
  are no longer on the public critical path. The $k=2$ route survives as
  important support infrastructure and as a local frontier file, but it is not
  the first blocker for the top-level theorem anymore.
...
\item Issue~\#48 has been resolved on the active OS route by moving the true
  boundary-value trust boundary into the SCV layer. The pure remaining
  non-QFT-specific trust boundary is now the axiom
  \leanname{tube\_boundaryValueData\_of\_polyGrowth} in
  \leanfile{SCV/TubeBoundaryValues.lean}.
...
\paragraph{E-to-R active public route.}
\begin{enumerate}
\item The semigroup/Hilbert-space support layer is established and spread across
  \leanfile{OSToWightmanSemigroup.lean},
  \leanfile{OSToWightmanK2BaseStep.lean},
  \leanfile{OSToWightmanK2Density.lean},
  \leanfile{OSToWightmanTubeIdentity.lean}, and the proved support file
  \leanfile{K2VI1/Support.lean}. These files are route infrastructure, not the
  first public blockers.
\item \leanfile{OSToWightman.lean} contains the current upstream continuation
  blockers. The three live private subproblems there are:
  \begin{itemize}
  \item \leanname{exists\_acrOne\_productTensor\_witness}
  \item \leanname{acrOne\_productTensor\_witness\_euclidKernelPackage}
  \item \leanname{compactlySupported\_zeroDiagonal\_subset\_closure\_admissibleProductTensorSet}
  \end{itemize}
  Together they are the honest general-$k$ ACR(1) continuation gap above the
  solved semigroup/$k=2$ support layer, where ACR(1) means the first
  positive-time-difference continuation region \(C_k^{(1)}\).
\item \leanfile{SCV/TubeBoundaryValues.lean} contains the explicit pure
  SCV/functional-analytic trust boundary:
  \begin{itemize}
  \item \leanname{tube\_boundaryValueData\_of\_polyGrowth}
  \end{itemize}
  This is now the honest non-QFT-specific issue-\#48 trust boundary.
\item \leanfile{OSToWightmanBoundaryValues.lean} is the downstream
  boundary-value transfer lane. The active blockers there are:
  \begin{itemize}
  \item \leanname{bvt\_lorentz\_covariant}
  \item \leanname{bvt\_locally\_commutative}
  \item \leanname{bvt\_positive\_definite}
  \item \leanname{bvt\_hermitian}
  \item \leanname{bvt\_cluster}
  \end{itemize}
  These theorems are now reduced to honest local pairing / positivity /
  cluster seams rather than the older monolithic boundary-value packaging.
\end{enumerate}

\paragraph{E-to-R local frontier.}
The repository still keeps a compact $k=2$ frontier file,
\leanfile{K2VI1/Frontier.lean}, as a local experimental and packaging surface
for OS~II Section~VI.1. It is useful for route diagnostics and two-point
specializations, but it is no longer the first public blocker for
\leanname{os\_to\_wightman\_full}.
...
below. The key update relative to the March-23 note is that the first public
frontier is no longer the $k=2$ VI.1 file. The mainline now bottlenecks first
at the general-$k$ continuation file, then at the pure SCV boundary-value axiom,
and then at the boundary-value transfer theorems.
...
\node[proved] (k2b) at (4.6,0)
  {k=2 support / density / identity layer\\{\footnotesize\texttt{K2BaseStep + K2Density +}\\\footnotesize\texttt{K2VI1/Support + TubeIdentity}}};
\node[frontier] (gen) at (9.5,0)
  {General-k continuation\\{\footnotesize\texttt{OSToWightman.lean}}\\3 private live theorems};
\node[pending] (tbv) at (14.4,0)
  {Pure SCV trust boundary\\{\footnotesize\texttt{SCV/TubeBoundaryValues.lean}}\\1 explicit axiom};
\node[pending] (bv) at (19.3,0)
  {Boundary-value transfer\\{\footnotesize\texttt{OSToWightman}\\\texttt{BoundaryValues.lean}}\\5 live theorems};
\draw[arr] (s) -- (k2b);
\draw[arr] (k2b) -- (gen);
\draw[arr] (gen) -- (tbv);
\draw[arr] (tbv) -- (bv);
...
\leanfile{OSToWightmanK2BaseStep.lean}, \leanfile{OSToWightmanK2Density.lean},
\leanfile{K2VI1/Support.lean}, \leanfile{OSToWightmanTubeIdentity.lean}
& compile-closed support layer &
Bochner package, difference-shell density, tube-identity lemmas, and the
proved support needed by the current continuation and transfer lanes. \\
\leanfile{OSToWightman.lean} & 3 private open theorems &
General-$k$ Osgood / product-tensor witness layer above the solved
semigroup/$k=2$ support core. \\
\leanfile{SCV/TubeBoundaryValues.lean} & 1 explicit axiom &
Pure tube-domain boundary-value existence from holomorphy plus global
polynomial growth. This is the current issue-\#48 trust boundary. \\
\parbox[t]{0.23\textwidth}{\ttfamily OStoWightman\\BoundaryValues.lean} & 5 open theorems &
Downstream Wightman-axiom transfer. The file already proves normalization and
translation invariance; the remaining seams are Lorentz covariance, locality,
positivity, hermiticity, and cluster. \\
\bottomrule
\end{longtable}
}

\paragraph{Important correction.}
Any diagram or discussion that routes the active $E \to R$ proof through either
the old fixed-probe two-point kernel theorem or the old
\leanname{boundary\_values\_tempered} blocker is now obsolete. The first route
was too strong and targeted the wrong theorem surface; the second was part of
issue~\#48 and has now been replaced by the pure SCV theorem surface in
\leanfile{SCV/TubeBoundaryValues.lean}.
...
\node[sdep] (holo) at (8.4,0) {\leanname{full\_analytic\_continuation\_with\_symmetry\_growth}\\proved, sorry-dependent};
\node[package] (k2b) at (12.6,0) {$k=2$ support package\\{\footnotesize\texttt{K2BaseStep + K2Density +}\\\footnotesize\texttt{K2VI1/Support}}};
\node[open] (genk) at (16.8,0) {\leanfile{OSToWightman.lean}\\3 private open theorems};
\node[axiomnode] (tbv) at (21.0,0) {\leanname{tube\_boundaryValueData\_of\_polyGrowth}\\pure SCV axiom};
\node[sdep] (bv) at (25.2,0) {\leanname{boundary\_values\_tempered}\\proved, sorry-dependent};
\node[open] (bvt) at (29.4,0) {\leanfile{OSToWightmanBoundaryValues.lean}\\5 open transfer theorems};
...
\noindent
In the $E \to R$ chart, the orange node marks the explicit pure SCV trust
boundary \leanname{tube\_boundaryValueData\_of\_polyGrowth}. The red nodes now
correspond to the genuinely live theorem seams: the three private continuation
subproblems in \leanfile{OSToWightman.lean} and the five transfer theorems in
\leanfile{OSToWightmanBoundaryValues.lean}.
...
sorry-free. This matters because the current OS route repeatedly packages mixed
time-translation and spatial-translation orbits, so it is useful to know that
the spatial group law itself is not part of the unresolved frontier.
...
\medskip\noindent
\textbf{What \(C_k^{(1)}\) means.}
The first continuation region \(C_k^{(1)}\), written in Lean as
\leanname{AnalyticContinuationRegion~d~k~1}, is the one-slice forward tube in
which only the \emph{time-difference} imaginary parts are constrained:
\[
C_k^{(1)}
=
\left\{
z \in \mathbb{C}^{k \times (d+1)}
\;\middle|\;
\Im(z_0^0) > 0,\;
\Im(z_j^0 - z_{j-1}^0) > 0 \text{ for } j=1,\dots,k-1
\right\}.
\]
In the formalization this region is also identified with the tube over the
positive time-difference cone after passing to flattened center/difference
coordinates. So when this note refers to the general-\(k\) ``ACR(1)'' problem,
the content is: construct a single holomorphic witness on this first
positive-time-difference tube, then continue and extend it coherently.

\begin{lstlisting}
-- OSToWightman.lean:2411 (PROVED)
...
\medskip\noindent
\textbf{Dependency-aware status.}
The main continuation theorems \leanname{full\_analytic\_continuation},
\leanname{full\_analytic\_continuation\_with\_growth}, and
\leanname{full\_analytic\_continuation\_with\_symmetry\_growth} are all present
in Lean. Their current trust boundary is no longer the old $k=2$ VI.1 frontier,
but the three private continuation subproblems in \leanfile{OSToWightman.lean}:
\leanname{exists\_acrOne\_productTensor\_witness},
\leanname{acrOne\_productTensor\_witness\_euclidKernelPackage}, and
\leanname{compactlySupported\_zeroDiagonal\_subset\_closure\_admissibleProductTensorSet}.
So this layer is best described as \emph{proved theorem surface sitting above a
small but honest general-$k$ continuation frontier}.


\section{The k=2 Base Step (OS~II, Theorem~4.1)}

This is still the initial nontrivial OS~II step, but it is no longer the first
top-level blocker for the public $E \to R$ route. The repository now splits the
$k=2$ lane into support, density, and a small local frontier:
\begin{itemize}
\item \leanfile{OSToWightmanK2BaseStep.lean} --- the Bochner/probe/support layer.
\item \leanfile{OSToWightmanK2Density.lean} --- flat-origin difference-shell
  density.
\item \leanfile{K2VI1/Support.lean} --- proved VI.1 support wrappers.
\item \leanfile{K2VI1/Frontier.lean} --- a compact local frontier / theorem
  surface for the $k=2$ time-parametric theorem.
\end{itemize}

The important update is that the $k=2$ story should now be read as a
\emph{stabilized support lane}. It remains mathematically important, and the
public two-point theorem surface is still documented here, but the first live
public blocker for \leanname{os\_to\_wightman\_full} has moved upward to the
general-$k$ continuation and boundary-value layers.

\medskip\noindent
\textbf{Mathematical content of the \(k=2\) base step.}
The \(k=2\) theorem is the first place where the abstract continuation witness
becomes a concrete Euclidean kernel. In the current route, its proof packages
four ingredients:
\begin{enumerate}
\item semigroup/continuation input producing a common holomorphic witness on the
  first continuation region;
\item translation invariance, which collapses the witness to difference-block
  dependence rather than absolute-position dependence;
\item swap symmetry, which lets one assemble a single real kernel \(K\) from
  the positive and negative time-ordering sectors;
\item density and tube-identity arguments, which extend reproduction from the
  natural dense shell classes to all zero-diagonal two-point Schwartz tests.
\end{enumerate}
So the present role of the \(k=2\) lane is not to replace the general
continuation story, but to certify that the two-point Euclidean section already
has the expected kernel-theoretic form and can be used downstream as stable
support infrastructure.

\begin{lstlisting}
-- K2VI1/Frontier.lean:1731
...
The right way to read the $k=2$ material now is:
\begin{itemize}
\item the heavy support machinery is already packaged and exact-checks;
\item the small \leanfile{K2VI1/Frontier.lean} file remains valuable for local
  route experiments and explicit theorem surfaces;
\item however, the public top-level $E \to R$ critical path has moved on to the
  general-$k$ continuation file \leanfile{OSToWightman.lean} and the
  boundary-value transfer file \leanfile{OSToWightmanBoundaryValues.lean}.
\end{itemize}
...
\subsection{Current k=2 Status}
\label{sec:k2sorry}

The March-23 version of this note treated the $k=2$ VI.1 lane as the first
global public blocker. That is no longer the right picture.

\medskip\noindent
\textbf{What remains true.}
The $k=2$ route is still mathematically central. It explains where the probe
witnesses, Bochner measures, difference-shell densities, and tube-identity
lemmas come from. Those results continue to feed the later continuation and
boundary-value layers.

\medskip\noindent
\textbf{What changed.}
The current public blocker map is no longer organized around the old pair
\leanname{translatedProductShell\_boundary\_tendsto\_schwingerDifferencePositive\_of\_negativeApproxIdentity\_local}
and \leanname{exists\_k2\_timeParametric\_distributional\_assembly}. Those
results were useful for isolating the honest VI.1 content, but the active
project frontier has since moved to:
\begin{enumerate}
\item the three private continuation subproblems in
  \leanfile{OSToWightman.lean},
\item the pure SCV axiom \leanname{tube\_boundaryValueData\_of\_polyGrowth} in
  \leanfile{SCV/TubeBoundaryValues.lean},
\item the five remaining transfer theorems in
  \leanfile{OSToWightmanBoundaryValues.lean}.
\end{enumerate}

\medskip\noindent
So this subsection is now best read as \emph{historical/current support context}
for the general OS route, not as the current top-level blocker map.


\section{Boundary Values and Wightman Axiom Transfer}

Once the analytic continuation is established, the Wightman distributions are
defined as boundary values:
\[
...
\medskip\noindent
\textbf{Direction independence.}
The point of the boundary-value package is not merely that the limit exists for
some interior direction \(\eta\), but that the \emph{same} functional
\(\mathfrak{W}_n\) is recovered for every choice of \(\eta \in V_+\). In the
current repo this direction-independence is exactly what the pure SCV trust
boundary \leanname{tube\_boundaryValueData\_of\_polyGrowth} supplies.

\medskip\noindent
\textbf{Current route correction (issue \#48).}
Earlier versions of this note described
\leanname{boundary\_values\_tempered} as the live blocker and routed it through
the SCV \leanname{uniform\_bound} abstraction. That is now obsolete. The active
OS path has been corrected as follows:
\begin{enumerate}
\item the chosen continuation now comes from
  \leanname{full\_analytic\_continuation\_with\_symmetry\_growth};
\item the pure boundary-value existence step has been moved into the SCV layer
  as the axiom \leanname{tube\_boundaryValueData\_of\_polyGrowth};
\item \leanname{boundary\_values\_tempered} is now a theorem, not the live
  frontier;
\item the remaining blockers in
  \leanfile{OSToWightmanBoundaryValues.lean} are the downstream transfer
  statements for Lorentz covariance, locality, positivity, hermiticity, and
  cluster.
\end{enumerate}

\begin{lstlisting}
-- OSToWightmanBoundaryValues.lean:190
-- The continuous boundary-value witness for the chosen continuation.
theorem boundary_values_tempered
    ...
    exists (W_n : SchwartzNPoint d n -> C)
      ...
      (forall (σ : Equiv.Perm (Fin n)) (z : Fin n -> Fin (d + 1) -> C),
        F_analytic (fun j => z (σ j)) = F_analytic z) /\
      (forall (z : Fin n -> Fin (d + 1) -> C) (a : Fin (d + 1) -> C),
        F_analytic (fun j => z j + a) = F_analytic z)
\end{lstlisting}

\noindent\textbf{What is already done.}
Inside \leanfile{OSToWightmanBoundaryValues.lean}, the project already proves:
\begin{itemize}
\item the boundary-value packaging theorem \leanname{boundary\_values\_tempered};
\item normalization \leanname{bvt\_normalized};
\item translation invariance \leanname{bvt\_translation\_invariant};
\item the generic transfer wrappers that reduce the remaining axioms to honest
  local analytic packages.
\end{itemize}

\noindent\textbf{What remains.}
The live top-level theorems in the file are now:
\begin{itemize}
\item \leanname{bvt\_lorentz\_covariant} --- reduced to a pointwise positive-ray
  covariance package \leanname{hF\_lorentz};
\item \leanname{bvt\_locally\_commutative} --- reduced to a canonical-ray swap
  pairing package \leanname{hF\_swap\_pairing};
\item \leanname{bvt\_positive\_definite} --- reduced to the direct positivity
  package \leanname{hW\_pos};
\item \leanname{bvt\_hermitian} --- reduced to a pointwise negative-ray package
  \leanname{hF\_neg};
\item \leanname{bvt\_cluster} --- still open as the broadest remaining transfer
  theorem.
\end{itemize}

\medskip\noindent
\textbf{Current file status.}
\leanfile{OSToWightmanBoundaryValues.lean} is now a downstream transfer file in
the strongest sense. The package \leanname{boundary\_values\_tempered} already
produces the boundary-value distribution and the chosen forward-tube witness.
The only remaining work is the honest transfer of the remaining Wightman axioms.

\medskip\noindent
\textbf{Dependency-aware status.}
This file is now best read as a stack of \emph{proved wrappers above five honest
local seams}. In particular, the note should no longer describe
\leanname{boundary\_values\_tempered} as the live blocker.

\subsection{The Main Bridge Theorems}

\begin{lstlisting}
-- OSToWightmanBoundaryValues.lean:1403
...
\begin{lstlisting}
-- OSToWightmanBoundaryValues.lean:1382
-- (declaration present in Lean; currently sorry-dependent)
...
\section{Production Axioms (8 explicit axioms)}

As of 2026-03-31, the tracked production tree contains exactly eight explicit
...
\medskip\noindent
\textbf{Current grouping.}
The eight explicit axioms now split naturally into three groups:
\begin{itemize}
\item \textbf{nuclear / multilinear support:}
  \leanname{schwartz\_nuclear\_extension},
  \leanname{exists\_continuousMultilinear\_ofSeparatelyContinuous};
\item \textbf{SCV / Fourier--Laplace support:}
  \leanname{semigroupGroup\_bochner},
  \leanname{laplaceFourier\_measure\_unique},
  \leanname{vladimirov\_tillmann},
  \leanname{distributional\_cluster\_lifts\_to\_tube},
  \leanname{tube\_boundaryValueData\_of\_polyGrowth};
\item \textbf{flagged $R \to E$ BHW bridge:}
  \leanname{reduced\_bargmann\_hall\_wightman\_of\_input}.
\end{itemize}
...
files, not to the current public frontier itself.

\subsection{Laplace--Fourier Uniqueness}

\begin{lstlisting}[style=leanaxiom]
-- SemigroupGroupBochner.lean:59 (AXIOM)
axiom laplaceFourier_measure_unique {d : N}
    {mu nu : Measure (R * (Fin d -> R))}
    [IsFiniteMeasure mu] [IsFiniteMeasure nu]
    (hmu_nonneg : mu (Set.prod (Set.Iio 0) Set.univ) = 0)
    (hnu_nonneg : nu (Set.prod (Set.Iio 0) Set.univ) = 0)
    (hEq : forall (t : R) (a : Fin d -> R), 0 <= t ->
      integral (fun p =>
        Complex.exp (-(t * p.1 : C)) *
        Complex.exp (Complex.I *
          (Finset.sum Finset.univ (fun i => p.2 i * a i) : C))) mu
      =
      integral (fun p =>
        Complex.exp (-(t * p.1 : C)) *
        Complex.exp (Complex.I *
          (Finset.sum Finset.univ (fun i => p.2 i * a i) : C))) nu) :
    mu = nu
\end{lstlisting}

\noindent\textbf{Mathematical meaning:}
This is the uniqueness half of the same Laplace--Fourier representation story.
It is not itself a current front-line blocker, but it is part of the trusted
spectral support layer feeding the semigroup and $k=2$ witness packages.
...
side rather than on the current issue-\#48 front.

\subsection{Distributional Cluster Lift}

\begin{lstlisting}[style=leanaxiom]
-- VladimirovTillmann.lean:133 (AXIOM)
axiom distributional_cluster_lifts_to_tube {n1 n2 d : N}
    (C_cone : Set (Fin (n1 + n2) -> Fin (d+1) -> R))
    ...
\end{lstlisting}

\noindent\textbf{Mathematical meaning:}
This is a separate SCV/cluster bridge axiom used downstream in the cluster lane.
It is distinct from the newer issue-\#48 boundary-value existence axiom below.

\subsection{Tube Boundary Values from Polynomial Growth}

\begin{lstlisting}[style=leanaxiom]
-- TubeBoundaryValues.lean:745 (AXIOM)
axiom tube_boundaryValueData_of_polyGrowth
    {n d : N}
    (C : Set (Fin n -> Fin (d + 1) -> R))
    (hC_open : IsOpen C) (hC_conv : Convex R C)
    (hC_cone : IsCone C) (hC_salient : IsSalientCone C)
    {F : (Fin n -> Fin (d + 1) -> C) -> C}
    (hF_hol : DifferentiableOn C F (TubeDomainSetPi C))
    (C_bd : R) (N : N) (hC : 0 < C_bd)
    (hF_growth : forall z in TubeDomainSetPi C,
      ‖F z‖ <= C_bd * (1 + ‖z‖) ^ N) :
    exists (W : SchwartzMap (Fin n -> Fin (d + 1) -> R) C ->L[C] C),
      forall (φ : SchwartzMap (Fin n -> Fin (d + 1) -> R) C)
        (η : Fin n -> Fin (d + 1) -> R),
        η ∈ C ->
        Filter.Tendsto
          (fun ε : R => ∫ x : Fin n -> Fin (d + 1) -> R,
            F (fun k μ => ↑(x k μ) + (ε : C) * ↑(η k μ) * Complex.I) * φ x)
          (nhdsWithin 0 (Set.Ioi 0))
          (nhds (W φ))
\end{lstlisting}

\noindent\textbf{Mathematical meaning:}
This is the new pure SCV / functional-analytic trust boundary introduced by the
issue-\#48 correction. It replaces the false dependence of the active OS route
on the older \leanname{uniform\_bound} abstraction. It is intentionally stated
as a non-QFT-specific theorem about holomorphic functions on tube domains with
global polynomial growth. It is the forward boundary-value existence statement
in the style of Vladimirov's tube-domain theory~\cite{Vlad}. Our hypothesis is
stronger than the classical one used in that literature: we assume a global
polynomial bound on the whole tube, rather than permitting additional
inverse-power blow-up near \(\partial C\).
...
\item \leanname{os\_to\_wightman} is proved but sorry-dependent, because it sits
  downstream of the three private continuation blockers in
  \leanfile{OSToWightman.lean}, the pure SCV axiom
  \leanname{tube\_boundaryValueData\_of\_polyGrowth}, and the five remaining
  transfer theorems in \leanfile{OSToWightmanBoundaryValues.lean}.
\end{itemize}
...
\S4.1 Analytic cont.\ & \leanname{full\_analytic\_continuation\_with\_symmetry\_growth} & proved, sorry-dependent \\
\S4.2 Boundary values & \leanname{boundary\_values\_tempered} & proved, sorry-dependent \\
\S4.2 Lorentz covariance & \leanname{bvt\_lorentz\_covariant} & open (local \leanname{hF\_lorentz}) \\
\S4.2 Spectrum & \leanname{spectrum\_condition} & Open downstream \\
\S4.3 Positivity & \leanname{bvt\_positive\_definite} & open (local \leanname{hW\_pos}) \\
\S4.4 Cluster & \leanname{bvt\_cluster} & \sorry \\
\S4.5 Locality & \leanname{bvt\_locally\_commutative} & open (local \leanname{hF\_swap\_pairing}) \\
\S4.5 Hermiticity & \leanname{bvt\_hermitian} & open (local \leanname{hF\_neg}) \\
...
Present theorem surface; no longer the\\first public blocker \\
General-$k$ first-step assembly &
\leanname{schwinger\_continuation\_base\_step\_acrOne\_productTensor\_assembly\_with\_translationInvariant} &
proved theorem surface; sorry-dependent\\(3 private blockers in\\\leanfile{OSToWightman.lean}) \\
...
\S VI.1: sequence regularization / assembly lane & \leanfile{K2VI1/Support.lean}, \leanfile{K2VI1/Frontier.lean} & Local $k=2$ route support / frontier \\
\S VI.2: boundary values from tube growth & \leanname{tube\_boundaryValueData\_of\_polyGrowth} & Axiom (pure SCV trust boundary) \\
...
Every later OS semigroup, spatial-translation, continuation, and boundary-value
argument is phrased in
...
\leanfile{OSToWightmanK2BaseStep.lean}, \leanfile{K2VI1/Support.lean}, and
\leanfile{OSToWightman.lean}.
...
difference variables. This is why the current continuation and boundary-value
frontier is still genuinely an OS semigroup problem rather than a separate
Euclidean-kernel problem.
...
\medskip\noindent
\textbf{Critical-path role.}
This layer is already proved. The remaining open work is not about existence of
the holomorphic object but about assembling the many-variable continuation and
transporting its boundary values to the final Wightman-axiom package.
...
So this layer is stable support infrastructure for the current OS route.
...
\medskip\noindent
\textbf{Repo realization.}
In the current production route this lane now lives squarely in
\leanfile{OSToWightman.lean}. The file packages the chosen one-slice witness,
its permutation/translation symmetries, and its global polynomial-growth
package, then uses density and Osgood-style closure arguments to promote the
Euclidean reproduction formula from admissible product tensors to all
zero-diagonal tests.

\medskip\noindent
\textbf{Current frontier above the solved base.}
The live private blockers in \leanfile{OSToWightman.lean} are now exactly:
\begin{enumerate}
\item \leanname{exists\_acrOne\_productTensor\_witness},
\item \leanname{acrOne\_productTensor\_witness\_euclidKernelPackage},
\item \leanname{compactlySupported\_zeroDiagonal\_subset\_closure\_admissibleProductTensorSet}.
\end{enumerate}
This is a cleaner frontier than the March-23 version of the note: the old
monolithic theorem-2 blocker has been split into one witness-construction
problem, one Euclidean-kernel packaging problem, and one dense-subset closure
problem.

\medskip\noindent
\textbf{Dependency-aware status.}
The big public continuation declarations in this lane, such as
\leanname{full\_analytic\_continuation},
\leanname{full\_analytic\_continuation\_with\_growth}, and
\leanname{full\_analytic\_continuation\_with\_symmetry\_growth}, are present
but remain sorry-dependent through exactly those three private subproblems.
...
The current repository splits this lane into:
\begin{itemize}
\item \leanfile{OSToWightmanK2BaseStep.lean} for Bochner/probe support,
\item \leanfile{OSToWightmanK2Density.lean} for flat-origin shell density,
\item \leanfile{K2VI1/Support.lean} for the proved VI.1 support wrappers,
\item \leanfile{K2VI1/Frontier.lean} for the compact local $k=2$ theorem
  surface.
\end{itemize}

\medskip\noindent
\textbf{Current route role.}
This lane should no longer be described as the first public frontier. Instead,
it is now a stabilized support layer that continues to feed the many-variable
continuation and boundary-value proofs. The project still keeps a small local
$k=2$ frontier file because the two-point theorem surface remains mathematically
useful and helpful for route diagnostics, but the global blocker map has moved on.

\medskip\noindent
\textbf{Dependency-aware status.}
The right summary is: the $k=2$ route remains important and active as
infrastructure, but it is no longer the top-level public bottleneck for
\leanname{os\_to\_wightman\_full}.

\textbf{Files:} \leanname{OSToWightmanK2BaseStep.lean},
\leanname{OSToWightmanK2Density.lean},
\leanname{K2VI1/Support.lean}, and \leanname{K2VI1/Frontier.lean}.
...
still sits immediately below the local $k=2$ theorem surface and the later
general-$k$ continuation layer.
...
\medskip\noindent
\textbf{Issue-\#48 correction.}
This subsection now needs one important warning that earlier versions of the
note missed: the active OS path no longer routes tempered boundary values
through the older SCV \leanname{uniform\_bound} abstraction in
\leanfile{SCV/LaplaceSchwartz.lean}. The honest trust boundary for the current
OS route has been moved into \leanfile{SCV/TubeBoundaryValues.lean} as the axiom
\leanname{tube\_boundaryValueData\_of\_polyGrowth}. So the Fourier--Laplace files
remain important background infrastructure, but they are not the active
issue-\#48 frontier anymore.
...
again whenever the project returns to the three private general-$k$ theorems in
\leanfile{OSToWightman.lean}.
...
\leanfile{OSToWightmanSemigroup.lean}
$\to$
\leanfile{OSToWightmanK2BaseStep.lean} / \leanfile{K2VI1/Support.lean}
$\to$
\leanfile{OSToWightman.lean}
$\to$
\leanfile{SCV/TubeBoundaryValues.lean}
...
\leanfile{OSToWightmanBoundaryValues.lean}.
\end{center}
The semigroup and $k=2$ files are now support layers, not the first public
blocker files. The live route above them is:

\medskip\noindent
\textbf{Blocker 1: general-$k$ ACR(1) witness production.}
The first private open theorem is
\leanname{exists\_acrOne\_productTensor\_witness} in
\leanfile{OSToWightman.lean}. This is the honest OS~II one-slice/Osgood problem:
construct a scalar holomorphic witness on \(C_k^{(1)}\) that already reproduces
the Schwinger functional on admissible factorized zero-diagonal tests. Here
\(C_k^{(1)}\) is the first continuation region from the previous section: the
open tube in which the imaginary time differences are strictly positive.

\medskip\noindent
\textbf{Blocker 2: Euclidean kernel packaging.}
The second private open theorem is
\leanname{acrOne\_productTensor\_witness\_euclidKernelPackage}. This is the
clean route-corrected issue: package the Euclidean section of the chosen witness
as a measurable polynomially bounded kernel on real configurations.

\medskip\noindent
\textbf{Blocker 3: admissible dense subset closure.}
The third private open theorem is
\leanname{compactlySupported\_zeroDiagonal\_subset\_closure\_admissibleProductTensorSet}.
This is the honest density problem left after the theorem-2 wrapper was split
properly.

\medskip\noindent
\textbf{Blocker 4: pure SCV boundary-value existence.}
The next trust boundary is the explicit axiom
\leanname{tube\_boundaryValueData\_of\_polyGrowth} in
\leanfile{SCV/TubeBoundaryValues.lean}. This is the current non-QFT-specific
issue-\#48 trust boundary.

\medskip\noindent
\textbf{Blocker 5: boundary-value transfer.}
Finally, \leanfile{OSToWightmanBoundaryValues.lean} contains the five remaining
transfer theorems:
\leanname{bvt\_lorentz\_covariant},
\leanname{bvt\_locally\_commutative},
\leanname{bvt\_positive\_definite},
\leanname{bvt\_hermitian}, and
\leanname{bvt\_cluster}.
...
\begin{enumerate}
\item \textbf{Keep the continuation trust boundary honest.}
  The next upstream work should stay in \leanfile{OSToWightman.lean} until the
  three private blockers above are either proved or further split honestly.

\item \textbf{Treat the SCV boundary-value theorem explicitly as a trust boundary.}
  The project should not drift back to the rejected
  \leanname{uniform\_bound} route. The current trust boundary is the pure axiom
  \leanname{tube\_boundaryValueData\_of\_polyGrowth}, and the note should keep
  that fact explicit.

\item \textbf{Continue shrinking the BV transfer seams one at a time.}
  The current recommended order is hermiticity, locality, Lorentz covariance,
  positivity, and then cluster. This matches the actual theorem surfaces in
  \leanfile{OSToWightmanBoundaryValues.lean}.

\item \textbf{Keep the $k=2$ lane as support, not as the main blocker story.}
  The $k=2$ files remain essential, but development planning should no longer
  describe them as the first public frontier unless the theorem graph changes
  again.
\end{enumerate}
...
\item the $k=2$ support layer in \leanfile{OSToWightmanK2BaseStep.lean},
  \leanfile{OSToWightmanK2Density.lean}, and \leanfile{K2VI1/Support.lean};
\item the tube-identity bridges in \leanfile{OSToWightmanTubeIdentity.lean};
...
still not be fully dependency-clean, because the production tree currently
contains eight explicit axioms:
\begin{itemize}
...
\item \leanname{laplaceFourier\_measure\_unique}
\item \leanname{vladimirov\_tillmann}
\item \leanname{distributional\_cluster\_lifts\_to\_tube}
\item \leanname{tube\_boundaryValueData\_of\_polyGrowth}
...
\item no return to the false \leanname{uniform\_bound} boundary-value route;
\item no Banach-valued Bochner infrastructure for Schwartz spaces merely to make
  a non-OS shortcut typecheck;
\item no new production axiom introduced merely to rename a live continuation,
  boundary-value, or BHW blocker.
\end{itemize}
...
\begin{enumerate}
\item close \leanname{bvt\_hermitian} by proving the local package
  \leanname{hF\_neg};
\item close \leanname{bvt\_locally\_commutative} by proving the local package
  \leanname{hF\_swap\_pairing};
\item close \leanname{bvt\_lorentz\_covariant} by proving the local package
  \leanname{hF\_lorentz};
\item attack \leanname{bvt\_positive\_definite} and then
  \leanname{bvt\_cluster};
\item in parallel or after that, return to \leanfile{OSToWightman.lean} and
  continue shrinking the three private continuation blockers;
\item after route completion, begin a separate axiom-elimination / trust-boundary
  cleanup phase, starting with the pure SCV axiom
  \leanname{tube\_boundaryValueData\_of\_polyGrowth}.
\end{enumerate}
...
\section{Current Sorry / Axiom Snapshot (2026-03-31)}

\textbf{Explicit axioms in tracked production tree: 8.}
\begin{itemize}
\item \leanname{schwartz\_nuclear\_extension}
\item \leanname{exists\_continuousMultilinear\_ofSeparatelyContinuous}
\item \leanname{semigroupGroup\_bochner}
...
\textbf{Rough raw \sorry{} token count in the tracked tree: 62.}
This count was obtained mechanically from lines matching
\texttt{\string^\string\s*sorry\$} in the tracked \texttt{OSReconstruction/}
tree on 2026-03-31. It is a lexical snapshot, not a theorem-level audit; in
particular, it may count commented local-frontier lines and therefore should not
be confused with the active public blocker count.

\begin{itemize}
\item \textbf{Wightman/} --- 24
\item \textbf{SCV/} --- 2
\item \textbf{ComplexLieGroups/} --- 2
\item \textbf{vNA/} --- 34
\end{itemize}

\textbf{Current public reconstruction front.}
The active public route-relevant distribution is now:
\begin{itemize}
\item \textbf{$E \to R$}
  \begin{itemize}
  \item \leanfile{OSToWightman.lean} --- 3 private open theorems
  \item \leanfile{SCV/TubeBoundaryValues.lean} --- 1 explicit axiom
  \item \leanfile{OSToWightmanBoundaryValues.lean} --- 5 open theorems
  \end{itemize}
\item \textbf{$R \to E$}
  \begin{itemize}
  \item \leanfile{BHWTranslation.lean} --- 1
  \item \leanfile{ForwardTubeLorentz.lean} --- 1
  \item \leanfile{SchwingerAxioms.lean} --- 2
  \end{itemize}
...
the $E \to R$ route now bottlenecks first at the general-$k$ continuation
subproblems in \leanfile{OSToWightman.lean}, then at the pure SCV trust
boundary in \leanfile{SCV/TubeBoundaryValues.lean}, and only after that at the
five remaining transfer theorems in
\leanfile{OSToWightmanBoundaryValues.lean}.
...
\medskip\noindent
The $k=2$ files deserve one explicit clarification here. They remain important
route infrastructure and still contain local theorem surfaces, but they should
no longer be read off this snapshot as the first public blocker for the
top-level OS reconstruction theorem.

The vNA module (34 sorrys) contains Stone's theorem, Tomita--Takesaki
...
MIT Press, 1966.
(Russian original: Nauka, Moscow, 1964.)
\end{thebibliography}
