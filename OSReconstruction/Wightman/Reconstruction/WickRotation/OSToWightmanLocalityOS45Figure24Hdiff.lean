          ...
          have hzero_edge_pointwise :
            ...
            have hsource_edge :
              ...
              /-
                ...
                source support.  Open the checked initial-overlap branch
                packet and reduce the pointwise common-edge equality to the
                paper's compact Wick-section transport for the concrete
                adjacent `(4.12)` branch.
              -/
              rcases
                  BHW.os45CommonEdge_initialSectorOverlap_traces_except_adjacentWick
                    (d := d) hd OS lgc (P := P)
              ...
              rcases htail with ⟨hFadj_wick_extendF, htail⟩
              rcases htail with ⟨hFord_common, htail⟩
              rcases htail with ⟨hFadj_common, _hFadj_seed_trace⟩
              have hwick_pairing :
                  ∀ ψ : SchwartzNPoint d n,
                    ...
                    ∫ v : NPointDomain d n,
                      Ford (fun k => wickRotatePoint (v k)) * ψ v := by
                ...
                have htransported_wick_pairing :
                  ...
                  have hInitialOverlap_eq :
                    ...
                    have hLocalEOW_seed :
                      ...
                      have hE_open : IsOpen E := by
                        simpa [E, e] using
                          e.toHomeomorph.isOpenMap U hU_open
                      have hE_sub :
                          E ⊆ BHW.os45FlatCommonChartEdgeSet d n P
                            (1 : Equiv.Perm (Fin n)) := by
                        rintro x ⟨v, hvU, rfl⟩
                        exact
                          (BHW.os45CommonEdgeFlatCLE_mem_edgeSet_iff
                            d n P (1 : Equiv.Perm (Fin n)) v).mpr
                            (hU_sub hvU)
                          ...
                          calc
                            ...
                            _ = z0 := by
                                  ...
                                  simp [BHW.os45FlatCommonChartSourceSide,
                                    e, x0]
                        exact hcommon_eq_flat.symm
                      have hz0Ucx : z0 ∈ Ucx := by
                        rw [hz0_common]
                        exact hcommon_mem u hu
                      have hm : 0 < BHW.os45FlatCommonChartDim d n :=
                        BHW.os45FlatCommonChartDim_pos_of_adjacent d n hi
                      obtain ⟨hCone_open, _hCone_convex, _hCone_zero,
                          _hCone_smul, hCone_nonempty⟩ :=
                        BHW.os45FlatCommonChartCone_eowReady d n
                      obtain ⟨ys, hys_mem, hys_li⟩ :=
                        open_set_contains_basis hm
                          (BHW.os45FlatCommonChartCone d n)
                          hCone_open hCone_nonempty
                      let T : SchwartzMap
                          (BHW.OS45FlatCommonChartReal d n) ℂ →L[ℂ] ℂ :=
                        BHW.os45FlatCommonChart_ordinaryEdgeCLM hd OS lgc P
                      have hzero_pairings :
                          (∀ θ : SchwartzMap
                            ...
                            tsupport
                              (θ : BHW.OS45FlatCommonChartReal d n → ℂ) ⊆ E →
                            (∫ x : BHW.OS45FlatCommonChartReal d n,
                              BHW.os45FlatCommonChartBranch d n OS lgc
                                (1 : Equiv.Perm (Fin n))
                                (fun a => (x a : ℂ)) * θ x) =
                                ...
                                have hminus_to_zero :
                                  ...
                                  have hpair :=
                                    D.tendsto_sourceSide_extendF_sideZeroDiagonalCLM_pair
                                      ...
                                      hΩminus_open
                                      (by
                                        simpa [BHW.permAct] using hFminus_cont)
                                      hU_open subset_closure hU_compact
                                      η h0_plus_minus h0_minus θ hθ_compact hθE
                                  simpa [Lminus0] using hpair.2
                                have hminus_zero_eq_plus :
                                    Lminus0 = Lplus0 := by
                                  /-
                                    OS-I `(4.12)`--`(4.14)` endpoint boundary
                                    identification.  The neutral moving-test
                                    theorem above only transports the adjacent
                                    finite-height side branch to its own
                                    zero-height compact-test pairing `Lminus0`.
                                    The remaining mathematical input is the
                                    local Jost/EOW common-boundary statement
                                    that this adjacent zero-height pairing is
                                    the same ordinary common-edge pairing
                                    `Lplus0` on the compact source window.
                                  -/
                                  exact ?os45_OS414_adjacent_zeroHeight_boundary_identification
                                simpa [hminus_zero_eq_plus] using hminus_to_zero
                      ...
                      obtain ⟨W0, hW0_open, _hW0_pre, hz0W0,
                        ...
                        BHW.os45_BHWJost_initialSectorEqOn_open_of_flatCommonChart_localZeroHeight_pairingsCLM
                          (d := d) hd OS lgc (P := P)
                          hE_open hE_sub ys hys_mem hys_li x0 hx0 T
                          hzero_pairings.1 hzero_pairings.2
                      refine ⟨W0 ∩ Ucx, hW0_open.inter hUcx_open,
                        ⟨z0, hz0W0, hz0Ucx⟩, ?_, ?_⟩
                      · intro z hz
                        exact hz.2
                      · intro z hz
                ...
                calc
                  ...
                  _ =
                    ∫ v : NPointDomain d n,
                      bvt_F OS lgc n (fun k => wickRotatePoint (v k)) * ψ v :=
                      htransported_wick_pairing
                  _ =
                    ∫ v : NPointDomain d n,
                      Ford (fun k => wickRotatePoint (v k)) * ψ v := by
              ...
              exact
                BHW.os45CommonEdge_pulledRealBranches_eqOn_of_wickPairings
                  ...
                  hwick_pairing hFord_common hFadj_common u hu
            have hperm_common :
                BHW.extendF (bvt_F OS lgc n)
                    (BHW.permAct (d := d) P.τ
                      ((BHW.os45QuarterTurnCLE (d := d) (n := n)).symm
