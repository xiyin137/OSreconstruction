...
theorem OS45BHWJostHullData.os45CommonEdge_local414_integrals_of_OSI45_jostEOW_smearing
  ...
  have hsource_ext :
    ...
    /-
      ...
      The remaining producer is the residual transfer from the raw Wick-section
      source current to the deterministic BHW `extendF` side current on the same
      compact Figure-2-4 collar.  This is the local tempered-BV uniqueness step:
      the two currents have the same boundary distribution by the OS-I
      `(4.12)`--`(4.14)` source-current comparison, and the BHW side branches
      recover that distribution by Vladimirov boundary uniqueness.
    -/
    let Ext : ℝ → ℂ := fun ε =>
      (∫ u : NPointDomain d n,
        BHW.extendF (bvt_F OS lgc n)
    ...
    have hresidual :
      ...
      /-
        Exact remaining OS-I residual transfer.

        This is not a source-representation or Wick-pairing consumer.  It must
        compare, in compact-test pairing, the deterministic BHW side-current
        `Ext ε` with the raw source-current `Raw ε` already controlled by the
        OS source-side theorem.  The ordinary side should use the forward-tube
        boundary package for `bvt_F`; the adjacent side must use the Figure-2-4
        BHW/Vladimirov two-sector boundary uniqueness, since
        `permAct P.τ (wick u)` is not in the forward tube.
      -/
      have hExt :
          Tendsto Ext (𝓝[Set.Ioi 0] (0 : ℝ)) (𝓝 0) := by
        /-
          Deterministic BHW side-current boundary-value producer.

          This is the current-level Vladimirov/BHW input to be proved directly:
          identify the ordinary `+` branch and selected adjacent `-` branch as
          carrying the same Schwinger boundary distribution on the Figure-2-4
          Jost edge by the local Vladimirov tempered-BV uniqueness mechanism,
          then pull the two finite side-height flat-chart currents back through
          `os45FlatCommonChart_branch_integral_eq_sourceSide_extendF_sideZeroDiagonalCLM`.
          The raw OS source-current limit `hraw` below is already closed and
          must not be used as a source-representation substitute for this step.
        -/
        let Flat : ℝ → ℂ := fun ε =>
          (∫ x : BHW.OS45FlatCommonChartReal d n,
            BHW.os45FlatCommonChartBranch d n OS lgc
          ...
          ∫ x : BHW.OS45FlatCommonChartReal d n,
            BHW.os45FlatCommonChartBranch d n OS lgc
              ...
              φ x
        have hFlat :
            Tendsto Flat (𝓝[Set.Ioi 0] (0 : ℝ)) (𝓝 0) := by
          /-
            Flat-chart distributional EOW current limit.

            This is now the precise Vladimirov/BHW leaf: prove that the
            ordinary `+` and selected adjacent `-` flat side-branch currents
            have the same Schwinger boundary distribution on the Figure-2-4
            collar.  This should be produced by the OS-I `(4.12)`--`(4.14)`
            source-side Schwinger normalization plus local Vladimirov
            tempered-BV uniqueness on the common Jost edge, not by a
            source-representation, local-S-prime branch, or downstream EOW
            envelope.
          -/
          exact ?os45_vladimirov_flat_chart_side_current_schwinger_limit
        ...
        have hFlat_scaled :
            Tendsto (fun ε : ℝ => J⁻¹ * Flat ε)
              (𝓝[Set.Ioi 0] (0 : ℝ)) (𝓝 0) := by
          have hJ :
              Tendsto (fun _ : ℝ => J⁻¹)
                (𝓝[Set.Ioi 0] (0 : ℝ)) (𝓝 J⁻¹) :=
            tendsto_const_nhds
          simpa using hJ.mul hFlat
        refine hFlat_scaled.congr' ?_
        ...
        have hminus_pull :
          ...
          simpa [J, neg_mul, one_mul] using
            BHW.os45FlatCommonChart_branch_integral_eq_sourceSide_extendF_sideZeroDiagonalCLM
              ...
              hφE_minus hg_minus
        have hflat_eq : Flat ε = J * Ext ε := by
          change
            ((∫ x : BHW.OS45FlatCommonChartReal d n,
                BHW.os45FlatCommonChartBranch d n OS lgc
                  (1 : Equiv.Perm (Fin n))
          ...
          ring
        have hcancel : J⁻¹ * (J * Ext ε) = Ext ε := by
          rw [← mul_assoc, inv_mul_cancel₀ hJ_ne, one_mul]
        calc
          J⁻¹ * Flat ε = J⁻¹ * (J * Ext ε) := by rw [hflat_eq]
          _ = Ext ε := hcancel
      simpa using hExt.sub hraw
    have hsum :
        Tendsto (fun ε : ℝ => (Ext ε - Raw ε) + Raw ε)
          (𝓝[Set.Ioi 0] (0 : ℝ)) (𝓝 0) := by
      simpa using hresidual.add hraw
    refine hsum.congr' ?_
    exact Filter.Eventually.of_forall (fun ε => by ring)
      ...
      have hzero_pairing :
        ...
        have hsource_eq :
          ...
          · rcases
              ...
              ⟨Ucx, Ford, Fadj, hUcx_open, hUcx_connected,
            ...
            have hFadj_wick_raw :
              ...
              /-
                Exact Vladimirov/BHW adjacent-Wick trace transport leaf.

                The initial-overlap packet identifies `Fadj` at the ordinary
                Wick section with the deterministic adjacent BHW branch
                `extendF (bvt_F) ∘ permAct P.τ`.  The raw OS-I `(4.12)` seed
                chart identifies the adjacent branch at the seed with the
                selected Wick boundary value.  The remaining paper step is
                precisely the tempered boundary-value uniqueness transport
                from the seed chart to the ordinary Wick section.
              -/
              intro u hu
              calc
                Fadj (fun k => wickRotatePoint (u k)) =
            ...
            exact
              BHW.os45CommonEdge_pulledRealBranches_eqOn_of_E3_branchTraces
                ...
                hFord_wick hFadj_wick_raw hFord_common hFadj_common
          · intro u hu
                  ...
                  c *
                    (D.toSchwartzNPointCLM
                      (1 : Equiv.Perm (Fin n)) φ :
                        NPointDomain d n → ℂ) u)
                (hsource_eq u (hsource_support hu)).symm
          · have hφu :
                (D.toSchwartzNPointCLM
      ...
      /-
        ...
        leaf is therefore the local tempered-BV uniqueness statement that the
        ordinary plus and adjacent minus flat side branches have the same
        compact-test boundary distribution on this support window.
      -/
      have hflat_side_difference :
        ...
        /-
          ...
          This is the canonical local producer requested by the review note:
          transport the raw OS-I source-current comparison `hsource_current`
          through the local BHW Figure-2-4 wedge continuation and polynomial
          growth control, yielding equality of the compact-test side-boundary
          distributions for the deterministic flat branches.
        -/
        exact
      ...
      have hflat_zero :
        ...
        exact
          D.zeroHeightPairing_of_tendsto_flatCommonChart_sideBranch_difference_zero
            OS lgc η hηC φ hφ_compact hφE hflat_side_difference
