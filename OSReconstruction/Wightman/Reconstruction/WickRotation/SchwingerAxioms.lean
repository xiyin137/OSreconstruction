...
(`W_analytic_cluster_integral` and the wrapper
`wickRotatedBoundaryPairing_cluster`) is proved in
`RuelleClusterBound.lean` (downstream). The proof uses Ruelle 1962's
uniform-in-a polynomial bound (`ruelle_analytic_cluster_bound`) plus
the pointwise analytic cluster (`ruelle_analytic_cluster_pointwise`)
via dominated convergence.

The statements were originally sorry stubs here; deleted on 2026-05-06
once `RuelleClusterBound.lean` was wired up.

Refs: Osterwalder-Schrader 1973 §2, §4. Glimm-Jaffe Ch 19.
Araki-Hepp-Ruelle 1962 (Helv. Phys. Acta 35), Theorem 2.
-/
